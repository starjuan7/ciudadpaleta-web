...
async function loadData(){
  // Intentar primero desde GitHub API (siempre actualizado, sin caché de Pages)
  let loaded = false;
  try {
    const cfg = JSON.parse(localStorage.getItem('cp_gh') || '{}');
    if(cfg.token && cfg.user && cfg.repo){
      const url = `https://api.github.com/repos/${cfg.user}/${cfg.repo}/contents/data.json?ref=${cfg.branch||'main'}&_=${Date.now()}`;
      const r = await fetch(url, { headers: { Authorization: `Bearer ${cfg.token}`, Accept: 'application/vnd.github.raw' }});
      if(r.ok){
    ...
    console.warn('GitHub API fetch falló, usando data.json local', e);
  }
  if(!loaded){
    try {
...
function renderSabores(){
  ...
  const html = list.map(s=>{
    ...
    const drag = filtering ? '' : `draggable="true" ondragstart="dragStart(event,${idx},'sab')" ondragover="dragOver(event)" ondrop="dragDrop(event,${idx},'sab')" ondragend="dragEnd(event)" ondragleave="dragLeave(event)"`;
...
let dragSrc = null;
let dragKind = null;
let dragCtx = null; // {ciudadIdx} para puntos
...
function renderCiudades(){
  ...
  const html = DATA.ciudades.lista.map((c,i)=>{
    ...
    const puntosHtml = c.puntos.map((p,j)=>`
      <div draggable="true" ondragstart="dragStart(event,${j},'punto',{ciudadIdx:${i}})" ondragover="dragOver(event)" ondrop="dragDrop(event,${j},'punto',{ciudadIdx:${i}})" ondragend="dragEnd(event)" ondragleave="dragLeave(event)" style="display:grid;grid-template-columns:24px 70px 1fr auto;gap:8px;align-items:center;background:#fff;border:1px solid var(--border);border-radius:10px;padding:8px;margin-bottom:6px;">
        <div class="drag-handle" title="Arrastra para reordenar" style="cursor:grab;color:#94a3b8;font-size:1rem;line-height:1;text-align:center;user-select:none;">⋮⋮</div>
    ...
    const ciuImg = c._preview || c.img || '';
    return `
    <div draggable="true" ondragstart="dragStart(event,${i},'ciudad')" ondragover="dragOver(event)" ondrop="dragDrop(event,${i},'ciudad')" ondragend="dragEnd(event)" ondragleave="dragLeave(event)" data-idx="${i}" style="background:${c.featured?'linear-gradient(135deg,#fff8e1,#fef3c7)':'var(--bg)'};border:${c.featured?'2px solid #f59e0b':'1px solid var(--border)'};border-radius:14px;padding:14px;margin-bottom:14px;${c.featured?'box-shadow:0 4px 14px rgba(245,158,11,.15);':''}">
      <div style="display:grid;grid-template-columns:24px 90px 1fr auto auto;gap:10px;align-items:center;margin-bottom:10px;">
        <div class="drag-handle" title="Arrastra para reordenar" style="cursor:grab;color:#94a3b8;font-size:1.1rem;line-height:1;text-align:center;user-select:none;">⋮⋮</div>
        <div style="display:grid;gap:4px;">
          <div style="width:90px;height:64px;border-radius:10px;background:#f1f5f9 center/cover no-repeat;${ciuImg?`background-image:url('${esc(ciuImg)}');`:''}display:flex;align-items:center;justify-content:center;font-size:1.4rem;color:#94a3b8;cursor:pointer;border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,.06);" onclick="document.getElementById('ciuImgFile_${i}').click()" title="${ciuImg?'Cambiar foto':'Agregar foto de la ciudad'}">${ciuImg?'':'📷'}</div>
          <input type="text" value="${esc(c.emoji||'')}" placeholder="📍" style="width:90px;text-align:center;font-size:1.1rem;border:1px solid var(--border);border-radius:8px;padding:4px;" onchange="DATA.ciudades.lista[${i}].emoji=this.value;markDirty()">
          <input type="file" id="ciuImgFile_${i}" accept="image/*" style="display:none" onchange="handleCiudadImg(event,${i})">
        </div>
        ...
        <button onclick="toggleCiudadFeatured(${i})" title="${c.featured?'Quitar destacado':'Marcar como ciudad principal'}" style="background:${c.featured?'linear-gradient(135deg,#f59e0b,#fbbf24)':'#fff'};color:${c.featured?'#fff':'#94a3b8'};border:2px solid ${c.featured?'#f59e0b':'var(--border)'};border-radius:10px;padding:8px 12px;cursor:pointer;font-size:1.1rem;font-weight:800;display:flex;align-items:center;gap:4px;white-space:nowrap;transition:all .2s;">${c.featured?'👑 Principal':'☆ Marcar'}</button>
...
function addCiudad(){
  DATA.ciudades.lista.push({emoji:'📍',name:'',desc:'',featured:false,img:'',puntos:[]});
  markDirty();
  renderCiudades();
}

function toggleCiudadFeatured(i){
  if(!DATA.ciudades.lista[i]) return;
  DATA.ciudades.lista[i].featured = !DATA.ciudades.lista[i].featured;
  markDirty();
  renderCiudades();
}
...
async function handleCiudadImg(e, i){
  const file = e.target.files[0];
  if(!file) return;
  toast('Comprimiendo imagen...', 'info');
  const compressed = await compressImage(file);
  const slug = slugify(DATA.ciudades.lista[i].name || 'ciudad');
  const filename = `assets/ciudades/${slug}-${Date.now()}.jpg`;
  ...
  DATA.ciudades.lista[i].img = filename;
  DATA.ciudades.lista[i]._preview = compressed.dataUrl;
  markDirty();
  renderCiudades();
...
async function _handlePuntoImg_old(e, i, j){
}
...
async function doPublish(){
  ...
  try {
    if(pendingImages.length){
      ...
      // Procesar copia para no perder imágenes si una falla
      const remaining = [...pendingImages];
      while(remaining.length){
        const img = remaining[0];
        await ghPutFile(img.path, img.base64, msg+' (img)');
        remaining.shift();
        // Actualizar localStorage tras cada subida exitosa para no reintentar duplicados
        pendingImages = remaining.slice();
        localStorage.setItem('cp_pending_imgs', JSON.stringify(pendingImages));
      }
      done();
    }
    ...
    if(Array.isArray(cleanData.sabores)){
      cleanData.sabores.forEach(s => { delete s._preview; });
    }
...
function reloadFromGitHub(){
  if(dirty && !confirm('Tienes cambios sin publicar. ¿Recargar de todos modos? (se perderán)')) return;
  localStorage.removeItem('cp_data');
  localStorage.removeItem('cp_pending_imgs');
  location.reload();
}
