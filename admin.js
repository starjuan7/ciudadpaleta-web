...
function renderSabores(){
  ...
  const filtering = !!(search || cat!=='all');
  ...
  const html = list.map(s=>{
    ...
    const drag = filtering ? '' : `draggable="true" ondragstart="dragStart(event,${idx},'sab')" ondragover="dragOver(event)" ondragenter="dragEnter(event)" ondrop="dragDrop(event,${idx},'sab')" ondragend="dragEnd(event)" ondragleave="dragLeave(event)"`;
    const handle = filtering ? '' : '<div class="drag-handle" title="Arrastra para reordenar">⋮⋮</div>';
    return `<div class="sabor-row" ${drag} data-idx="${idx}">
      ${handle}
  ...
  const tip = filtering ? '<div style="text-align:center;padding:8px;color:var(--mute);font-size:.78rem;background:#fef3c7;border-radius:8px;margin-bottom:10px;">⚠️ Limpia la búsqueda y filtro para reordenar arrastrando</div>' : '<div style="text-align:center;padding:8px;color:var(--mute);font-size:.78rem;margin-bottom:10px;">💡 Arrastra los sabores para cambiar el orden en que aparecen en el sitio</div>';
  document.getElementById('saboresList').innerHTML = (list.length ? tip : '') + (html || '<div style="text-align:center;padding:30px;color:var(--mute);">Sin resultados</div>');
}
...
let dragKind = null;
let dragCtx = null;
let dragEnterCount = 0; // evita falsos dragleave al pasar sobre hijos

function dragStart(e, idx, kind, ctx){
  dragSrc = idx; dragKind = kind; dragCtx = ctx || null;
  dragEnterCount = 0;
  e.dataTransfer.effectAllowed = 'move';
  try{ e.dataTransfer.setData('text/plain', String(idx)); }catch(err){}
  // Marcar con pequeño delay para que el ghost del browser se vea normal
...
function dragEnd(e){
  ...
  dragSrc = null; dragKind = null; dragCtx = null;
}
function dragDrop(e, targetIdx, kind, ctx){
  e.preventDefault();
  dragEnterCount = 0;
  e.currentTarget.classList.remove('drag-over');
  if(dragSrc===null || dragKind!==kind || dragSrc===targetIdx) return;
  if(kind==='sab'){
    const arr = DATA.sabores;
    const item = arr.splice(dragSrc,1)[0];
    arr.splice(targetIdx,0,item);
    markDirty();
    renderSabores();
  } else if(kind==='punto'){
    const ci = (ctx && ctx.ciudadIdx!==undefined) ? ctx.ciudadIdx : (dragCtx && dragCtx.ciudadIdx);
    if(ci===undefined) return;
    const arr = DATA.ciudades.lista[ci].puntos;
    const item = arr.splice(dragSrc,1)[0];
    arr.splice(targetIdx,0,item);
    markDirty();
    renderCiudades();
...
function renderCiudades(){
  ...
  const TIPOS = ['Tiendita','Cafetería','Restaurante','Panadería','Gym/Spa','Evento','Otro'];
  ...
  const html = DATA.ciudades.lista.map((c,i)=>{
    ...
    const puntosHtml = c.puntos.map((p,j)=>{
      ...
      const tipoOptions = TIPOS.map(t=>`<option value="${t}" ${p.tipo===t?'selected':''}>${t}</option>`).join('');
      const imgBg = (p._preview||p.img) ? `background-image:url('${esc(p._preview||p.img)}');background-size:cover;background-position:center;` : '';
      const imgIcon = (p._preview||p.img) ? '' : '📷';
      return `
      <div draggable="true"
        ondragstart="dragStart(event,${j},'punto',{ciudadIdx:${i}})"
        ondragover="dragOver(event)"
        ondragenter="dragEnter(event)"
        ondrop="dragDrop(event,${j},'punto',{ciudadIdx:${i}})"
        ondragend="dragEnd(event)"
        ondragleave="dragLeave(event)"
        style="background:#fff;border:1px solid var(--border);border-radius:12px;margin-bottom:10px;overflow:hidden;opacity:${p.active===false?'0.55':'1'};transition:opacity .2s;">
        <div style="display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid var(--border);background:${p.active===false?'#fff8f8':'#fafafa'};">
          <div class="drag-handle" title="Arrastra para reordenar" style="cursor:grab;color:#94a3b8;font-size:1rem;user-select:none;padding:0 4px;">⋮⋮</div>
          <div onclick="document.getElementById('puntoImgFile_${i}_${j}').click()" title="Cambiar foto principal"
            style="width:60px;height:60px;border-radius:8px;flex-shrink:0;${imgBg}background-color:#f1f5f9;display:flex;align-items:center;justify-content:center;font-size:1.5rem;color:#94a3b8;cursor:pointer;border:2px dashed var(--border);">
            ${imgIcon}
          </div>
          <input type="file" id="puntoImgFile_${i}_${j}" accept="image/*" style="display:none" onchange="handlePuntoImg(event,${i},${j})">
          <div style="flex:1;display:grid;grid-template-columns:1fr auto;gap:6px;align-items:center;">
            <input type="text" value="${esc(p.name||'')}" placeholder="Nombre del lugar *"
              style="border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-weight:700;font-size:.92rem;width:100%;"
              onchange="DATA.ciudades.lista[${i}].puntos[${j}].name=this.value;markDirty()">
            <select style="border:1px solid var(--border);border-radius:6px;padding:7px 8px;font-size:.82rem;min-width:110px;"
              onchange="DATA.ciudades.lista[${i}].puntos[${j}].tipo=this.value;markDirty()">
              <option value="">Tipo...</option>${tipoOptions}
            </select>
          </div>
          <!-- Toggle visible/oculto -->
          <label title="${p.active===false?'Oculto en el sitio — clic para activar':'Visible en el sitio — clic para ocultar'}"
            style="display:flex;align-items:center;gap:5px;cursor:pointer;flex-shrink:0;background:${p.active===false?'#fee2e2':'#dcfce7'};border:1.5px solid ${p.active===false?'#fca5a5':'#86efac'};border-radius:20px;padding:4px 10px;font-size:.72rem;font-weight:800;color:${p.active===false?'#dc2626':'#16a34a'};user-select:none;white-space:nowrap;"
            onclick="DATA.ciudades.lista[${i}].puntos[${j}].active=DATA.ciudades.lista[${i}].puntos[${j}].active===false?true:false;markDirty();renderCiudades()">
            ${p.active===false?'⏸ Oculto':'✅ Visible'}
          </label>
          <button class="icon-btn del" onclick="DATA.ciudades.lista[${i}].puntos.splice(${j},1);markDirty();renderCiudades()" title="Eliminar punto" style="flex-shrink:0;">🗑️</button>
        </div>
        <div style="padding:12px;display:grid;gap:8px;">
          <div>
            <label style="font-size:.72rem;font-weight:700;color:var(--mute);display:block;margin-bottom:3px;">📍 Dirección</label>
            <input type="text" value="${esc(p.address||'')}" placeholder="Ej: Calle 5 de Mayo #123, Col. Centro, Puebla"
              style="width:100%;border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-size:.86rem;"
              onchange="DATA.ciudades.lista[${i}].puntos[${j}].address=this.value;markDirty()">
          </div>
          <div>
            <label style="font-size:.72rem;font-weight:700;color:var(--mute);display:block;margin-bottom:3px;">💬 Descripción (aparece en el modal del sitio)</label>
            <textarea placeholder="Ej: Tiendita familiar con refrigerador propio de Ciudad Paleta."
              style="width:100%;border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-size:.86rem;resize:vertical;min-height:56px;"
              onchange="DATA.ciudades.lista[${i}].puntos[${j}].desc=this.value;markDirty()">${esc(p.desc||'')}</textarea>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <div>
              <label style="font-size:.72rem;font-weight:700;color:var(--mute);display:block;margin-bottom:3px;">🕐 Horario</label>
              <input type="text" value="${esc(p.horario||'')}" placeholder="Lun–Dom 9:00–20:00"
                style="width:100%;border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-size:.85rem;"
                onchange="DATA.ciudades.lista[${i}].puntos[${j}].horario=this.value;markDirty()">
            </div>
            <div>
              <label style="font-size:.72rem;font-weight:700;color:var(--mute);display:block;margin-bottom:3px;">💬 WhatsApp</label>
              <input type="text" value="${esc(p.whatsapp||'')}" placeholder="522241250998"
                style="width:100%;border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-size:.85rem;"
                onchange="DATA.ciudades.lista[${i}].puntos[${j}].whatsapp=this.value;markDirty()">
            </div>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
            <div>
              <label style="font-size:.72rem;font-weight:700;color:var(--mute);display:block;margin-bottom:3px;">🗺️ Link Google Maps</label>
              <input type="url" value="${esc(p.maps||'')}" placeholder="https://maps.google.com/..."
                style="width:100%;border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-size:.82rem;"
                onchange="DATA.ciudades.lista[${i}].puntos[${j}].maps=this.value;markDirty()">
            </div>
            <div>
              <label style="font-size:.72rem;font-weight:700;color:var(--mute);display:block;margin-bottom:3px;">📸 Instagram (URL)</label>
              <input type="url" value="${esc(p.instagram||'')}" placeholder="https://instagram.com/..."
                style="width:100%;border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-size:.82rem;"
                onchange="DATA.ciudades.lista[${i}].puntos[${j}].instagram=this.value;markDirty()">
            </div>
          </div>
          <div>
            <label style="font-size:.72rem;font-weight:700;color:var(--mute);display:block;margin-bottom:3px;">🏷️ Etiquetas (separadas por coma)</label>
            <input type="text" value="${esc(tagsVal)}" placeholder="Ej: Centro, Estacionamiento, Pet friendly"
              style="width:100%;border:1px solid var(--border);border-radius:6px;padding:7px 10px;font-size:.85rem;"
              onchange="DATA.ciudades.lista[${i}].puntos[${j}].tags=this.value.split(',').map(t=>t.trim()).filter(Boolean);markDirty()">
            <div style="font-size:.7rem;color:var(--mute);margin-top:3px;">Aparecen como chips de color en el perfil del punto de venta</div>
          </div>
        </div>
      </div>`;
    }).join('');
...
function addPunto(i){
  ...
  DATA.ciudades.lista[i].puntos.push({name:'',address:'',whatsapp:'',img:'',desc:'',horario:'',tipo:'',maps:'',instagram:'',tags:[],active:true});
  markDirty();
  renderCiudades();
}
